...
for (i in 1:2){
  ...
  geom_line(aes(x = isotopeS[[i]]$date, 
            ...
            linewidth = 0.75) +
  scale_color_manual(values = c("cS (observed)" = "black", 
 ...
 plt[[i]] <- ggplot()+
  ...
  geom_line(data = revisedProcedure$fitGamma[[i]]$simulated, 
            aes(x = date, 
                y = simulated, 
                group = simulation, 
                color = "cS (revised procedure - top 100)"), 
            alpha = 0.8, 
            linewidth = 0.1)+
  geom_line(aes(x = isotopeS[[i]]$date, 
                y = as.numeric(traditionalProcedure$fitSineS[[i]]$fitted.values),  
  ...
  geom_line(data = subset(revisedProcedure$fitGamma[[i]]$simulated, 
            ...
            aes(x = date,
                y = simulated, 
                color = "cS (revised procedure - best)"),
            linewidth = 0.75)+
...
```{r, eval = TRUE, fig.height=2, fig.align = 'center'}
...
ggplot()+
  ...
  geom_point(data = isotopeInter$catchment[[1]]$dateInterVal[[4]]$S, 
             ...
             shape = 3, 
             alpha = 1)+
  scale_color_manual(values = c("daily sampling" = "red", 
                                "monthly sampling" = "blue"), 
