...
# POINTS indicate results from the traditional approach
# CROSSES indicate the best parameter set from the revised approach
ggplot(stack(revisedProcedure$fitGamma[[icatchment]]$parameterSet[,c(4,5,6)]))+
  geom_boxplot(aes(x = ind, y = values, fill = ind), outlier.shape = NA, show.legend = FALSE)+
  geom_point(data = addPoints, aes(x = ind, y = values, fill = ind), size = 4)+
  geom_point(data = bestPoints, aes(x = ind, y = values, fill = ind), size = 4, shape = 3)+
  labs(x = "", y = "Value", title = "Parameters of the transit times and Fyw (traditional vs. proposed procedures)") +  
